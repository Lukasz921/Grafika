namespace Proj1.Components
{
    internal class BezierSegment : Control
    {
        ...
        private void BezierSegment_Paint(object? sender, PaintEventArgs e)
        {
            var g = e.Graphics;
        ...
        private void BezierSegment_MouseDown(object? sender, MouseEventArgs e)
        {
            ...
            {
                polygon1.MouseDownBezier(mouse, true, null);
                Capture = true;
            }
            else if (e.Button == MouseButtons.Left && Parent is Polygon polygon2)
            {
                polygon2.MouseDownBezier(mouse, false, this);
                Capture = true;
            }
        }
        private void BezierSegment_MouseMove(object? sender, MouseEventArgs e)
        {
            if (Parent == null) return;
            if (Parent is Polygon polygon && polygon.IsDragging)
            {
                Point mouse = Parent.PointToClient(Cursor.Position);
                polygon.MouseMoveBezier(mouse, this);
            }
        }
        private void BezierSegment_MouseUp(object? sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left && Parent is Polygon polygon && polygon.IsDragging)
            {
                polygon.MouseUpBezier();
                Capture = false;
            }
        }
    }
