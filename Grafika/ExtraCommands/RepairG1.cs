...
namespace Grafika.ExtraCommands
{
    internal class RepairG1
    {
        public static void Repair(List<(BezierSegment, BezierSegment, Edge)> segments)
        {
            ...
            {
                Vertex? c1 = null;
                if (segment.Item3.V1.Type == VertexType.G1) c1 = segment.Item3.V1;
                if (segment.Item3.V2.Type == VertexType.G1) c1 = segment.Item3.V2;
                if (c1 == null) continue;
                ...
                double curDist = Math.Sqrt((b1.Center().X - c1.Center().X) * (b1.Center().X - c1.Center().X) + (b1.Center().Y - c1.Center().Y) * (b1.Center().Y - c1.Center().Y));
                var cC = c1.Center();
                var vC = v.Center();
                double dx = cC.X - vC.X;
                double dy = cC.Y - vC.Y;
        ...
        public static void Repair(BezierSegment b1)
        {
            Vertex? c1 = null;
            Vertex? v = null;
            if (b1.LeftVertex != null && b1.LeftVertex.Type == VertexType.G1)
            {
                c1 = b1.LeftVertex;
                v = c1.LeftEdge!.OtherVertex(c1);
            }
            if (b1.RightVertex != null && b1.RightVertex.Type == VertexType.G1)
            {
                c1 = b1.RightVertex;
                v = c1.RightEdge!.OtherVertex(c1);
            }
            ...
            double curDist = Math.Sqrt((v.Center().X - c1.Center().X) * (v.Center().X - c1.Center().X) + (v.Center().Y - c1.Center().Y) * (v.Center().Y - c1.Center().Y));
            var bC = b1.Center();
            var cC = c1.Center();
            double dx = cC.X - bC.X;
            double dy = cC.Y - bC.Y;
